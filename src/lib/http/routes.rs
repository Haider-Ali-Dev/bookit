...
use crate::database::models::{Email, Password};
use crate::{
    database::{models::Id, Database},
    http_models::models::{
        BookmarkFromDB, BookmarkStatus, CBookmark, CBookmarkRemove, CEmail, CSignIn, SRegister,
        User,
    },
};
...
async fn register(
    user: Json<CRegister>,
    db: &State<Database>,
) -> Result<Json<SRegister>, Json<ApiError>> {
    let hashed_password =
        hash(user.password.clone(), DEFAULT_COST).expect("Error while hashing the password");
    ...
    match user_data {
        ...
        Err(_) => Err(Json(ApiError::DatabaseError(format!(
            "Database error while getting the user."
        )))),
    }
}
...
async fn sign_in(user: Json<CSignIn>, db: &State<Database>) -> Result<Json<User>, Json<ApiError>> {
    let db_clone = db.pool.clone();
    let hashed_password = sqlx::query_as!(
        Password,
    ...
    match hashed_password {
        ...
        Err(_) => Err(Json(ApiError::DatabaseError(format!(
            "Error while getting the password from the database."
        )))),
    }
}
...
async fn bookmark_add(
    bookmark: Json<CBookmark>,
    database: &State<Database>,
) -> Result<Json<BookmarkStatus>, Json<ApiError>> {
    ...
    match id {
        Ok(_) => {
            ...
            match query {
                ...
                Err(_) => Err(Json(ApiError::DatabaseError(format!(
                    "Error while adding the boomark in the db"
                )))),
            }
        }
...
async fn bookmark_delete(
    bookmark: Json<CBookmarkRemove>,
    database: &State<Database>,
) -> Result<Json<BookmarkStatus>, Json<ApiError>> {
    let db_clone = database.pool.clone();
    let db_bookmark_data = sqlx::query_as!(
        Email,
    ...
    match db_bookmark_data {
        Ok(_) => {
            let del = sqlx::query!(
                "DELETE FROM bookmark WHERE bookmark_url = $1",
                bookmark.bookmark_url.clone()
            )
            .execute(&db_clone)
            .await;
            match del {
                Ok(_) => Ok(Json(BookmarkStatus { status: true })),
                Err(_) => Err(Json(ApiError::DatabaseError(
                    "Error while deleting".to_owned(),
                ))),
            }
        }
        Err(_) => Err(Json(ApiError::UserNotFound)),
...
#[post("/bookmark-list", data = "<bookmark>")]
async fn bookmark_list(
    bookmark: Json<CEmail>,
    database: &State<Database>,
) -> Result<Json<Vec<BookmarkFromDB>>, Json<ApiError>> {
    let db_clone = database.pool.clone();
    let bookmarks = sqlx::query_as!(
        BookmarkFromDB,
        "SELECT date,bookmark_name,bookmark_url from bookmark where email = $1",
        bookmark.email.clone()
    )
    .fetch_all(&db_clone)
    .await;
